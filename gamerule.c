...
void item_ex(void) {
	...
	gotoxy(MAP_X + (MAP_WIDTH - 14) / 2, MAP_Y + 4, "uuuuuuuuuuuuuuuuuuuuuuuuuuuu");
	Setcolor(7);
	gotoxy(MAP_X + (MAP_WIDTH - 14) / 2-3, MAP_Y + 6, "BATTERY + / -  ");
	gotoxy(MAP_X + (MAP_WIDTH - 14) / 2-3, MAP_Y + 8, "MORE STRIDER | oooo |  ");
	gotoxy(MAP_X + (MAP_WIDTH - 14) / 2-3, MAP_Y + 10, "HP");

	Setcolor(4);
	gotoxy(MAP_X + (MAP_WIDTH - 14) / 2 -1, MAP_Y + 10, "♥");

	Setcolor(7); gotoxy(MAP_X + (MAP_WIDTH - 14) / 2 + 1, MAP_Y + 10, "PLAYER LIFE");


	gotoxy(MAP_X + (MAP_WIDTH - 14) / 2-3, MAP_Y + 11, "SECRET PASSAGE");

	Setcolor(2);
	gotoxy(MAP_X + (MAP_WIDTH - 14) / 2 + 5, MAP_Y + 11, "P"); Setcolor(7);
	gotoxy(MAP_X + (MAP_WIDTH - 14) / 2 + 6, MAP_Y + 11, "TRNSPORT TO CONNECTED PASSAGE");
