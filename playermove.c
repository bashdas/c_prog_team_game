...
int movePlayer(struct player* player_info, struct items* item_array, struct strider* strider_info) {
	...
	int upper = 0, bottom = 7, left = 0, right = 14, cd = 0; // 플레이어 시야 배열을 얼만큼 돌릴 지
	int lastMove = -1;
	...
	while (1)
	{
		...
		HP_print(HP);
		int n = keyControl1();
		function_portal_Hard(&playerx, &playery, n);
		if (n == lastMove) {
			continue;
		}
		switch (n) {
		...
		cd = CDStrider(playerx, playery, strider_info);
		if (cd == FAIL_V) return FAIL_V;


		timek = timek + 0.5;

		gotoxy1(MAP_X * 2 + 4, MAP_Y + MAP_HEIGHT - 3);
		printf("남은 열쇠:2개");
		...
		if (100 - timek <= 0)
		{
			...
			printf("배터리가 방전되었습니다. HP가 1 감소합니다 (-. - )");
			Sleep(500);
			gotoxy1(MAP_X * 2 + 4, MAP_Y + MAP_HEIGHT - 4);
			printf("                                                  ");
			if (HP >= 1)
...
int playermoveNormal(struct player* player_info, struct items* item_array, struct strider* strider_info)
{
	...
	while (1)
	{
		...
		int n = keyControl1();
		function_portal(&playerx, &playery, n);
		if (n == lastMove) { // 방향키 입력시 2번 처리되는 문제 같은 값이 2번 들어오면 무시
			continue;
		}
		switch (n)
		{
		...
		if (isClear(item_array) == CLEAR_V)
		{
			gotoxy1(MAP_X * 2 + 4, MAP_Y + MAP_HEIGHT - 3);
			printf("                ");
			remove_portal();
			gotoxy1(MAP_X * 2 + 7, MAP_Y + MAP_HEIGHT - 3);
			return CLEAR_V;
		}
